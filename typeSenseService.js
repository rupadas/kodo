...
typeSenseService.fetchCollection = async () => {
    try {
        ...
        logger.error(`Errot at fetchCollection ${error?.message}`);
    }
};

typeSenseService.createCollection = async () => {
    try {
        const response = await axios.post('http://type-db:8108/collections',
            {
                "name": "sample",
                "fields": [
                    ...
                    { "name": "dateLastEdited", "type": "int64", "sort": true }
                ],
                "default_sorting_field": "dateLastEdited"
            },
            {
                headers: {
                    'X-TYPESENSE-API-KEY': 'xyz'
        ...
        logger.error(`Errot at create Collection ${error?.message}`);
    }
};

typeSenseService.createDocument = async () => {
    const samples = [
        {
    ...
    try {
        const updatedSamples = samples.map((item) => {
            item.dateLastEdited = +new Date(item.dateLastEdited);
            return JSON.stringify(item);
        }).join('\n');
        await axios.post(
            'http://type-db:8108/collections/sample/documents/import?action=create',
            updatedSamples,
            {
                headers: {
                    'X-TYPESENSE-API-KEY': 'xyz',
        ...
        logger.error(`Errot at create document ${error?.message}`);
    }
    return true;
};

typeSenseService.searchCollection = async (query) => {
    try {
        const response = await axios.get(`http://type-db:8108/collections/sample/documents/search?q=${query}&query_by=name&sort_by=name:asc&sort_by=dateLastEdited:desc`, {
            headers: {
                'X-TYPESENSE-API-KEY': 'xyz'
            }
        });
        return response.data.hits.map((x) => {
            x.document.dateLastEdited = new Date(x.document.dateLastEdited);
            return x;
        });
    } catch (error) {
        logger.error(`Errot at search Collection ${error?.message}`);
    }
};
